...
function parsePeriod(
  ...
  if (
    ...
    !VALID_PERIODS.includes(
      parsed as ProductAnalyticsPeriod
    )
  ) {
    return null;
  }
...
export async function GET(
  ...
  try {
    ...
    if (
      ...
      return ApiResponse.error(
        "No tienes permisos para consultar las analíticas de productos.",
        403
      );
    }
    ...
    return ApiResponse.success(
      analytics,
      "Analíticas de productos obtenidas correctamente."
    );
  } catch (
    error
  ) {
    Logger.error(
      "Error obteniendo analíticas de productos.",
      error
    );
